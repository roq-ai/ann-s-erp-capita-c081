...
import { getHumanResourcePlanById, updateHumanResourcePlanById } from 'apiSdk/human-resource-plans';
...
function HumanResourcePlanEditPage() {
  ...
  const handleSubmit = async (values: HumanResourcePlanInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateHumanResourcePlanById(id, values);
      mutate(updated);
      resetForm();
